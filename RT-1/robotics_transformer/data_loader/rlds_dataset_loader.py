...
class DecodeAndRandomResizedCrop(preprocess_spec.RandomMapTransform):
    ...
    resize_size: Tuple[int, int] = (256, 320)

    def _transform(self, features, seed):
    
        image = features["observation"]["image"]
        print(image[0])
        # jpeg_str = np.reshape(np.asarray(image.numpy()[0]).astype(np.str), (-1))
        # #print(tf.strings.as_string(image[0]))
        # print(jpeg_str)
        encode_img=tf.io.encode_jpeg(image[0])
        # shape = tf.io.extract_jpeg_shape(jpeg_str)
        # height, width, channels = tf.shape(image[0])
        # print(height.numpy())  # 输出：256
        # print(width.numpy())   # 输出：320
        # print(shape)
        shape = tf.io.extract_jpeg_shape(encode_img)
        
        raw_height, raw_width = shape[0], shape[1]
        raw_height = tf.cast(raw_height, tf.float32)
        raw_width = tf.cast(raw_width, tf.float32)
        print("raw_height",raw_height)
        ...
        #features["observation"]["rgb"]=tf.cast(features["observation"]["rgb"], tf.float32) / 255.0
        #features["observation"]["rgb"]= tf.image.resize(features["observation"]["rgb"], self.resize_size)
...
class TransformDict(preprocess_spec.RandomMapTransform):
    ...
    def _transform(self, features, seed):
        ...
        #'terminate_episode', 'world_vector', 'rotation_delta', 'gripper_closedness_action'
        action_lable = {
            "terminate_episode":features["action"]["terminate_episode"],
            "rotation_delta":features["action"]["rotation_delta"],
            "gripper_closedness_action":features["action"]["gripper_closedness_action"],
            "world_vector":features["action"]["world_vector"]}
        train_observation = {"image": features["observation"]["image"],
                             "natural_language_embedding": tf.cast(features['observation']['natural_language_embedding'],dtype=tf.float32)}
